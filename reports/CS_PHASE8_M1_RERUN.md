...
| **Pooled negative-control-feature kernel** ***[row renamed 2026-08-27 — it read "Negative-control-**probe** kernel (must be flat)", which names the wrong response]*** | *new* | **The 40 named negative-control PROBES — the pre-registered primary technical null (`PREREG_PHASE8_genesets.md` §11, `E_negative_control_probes`) — are FLAT: −0.0225 [−0.0527, +0.0078], p = 0.129. On its own primary response M1's A7 PASSES.** The gradient is in the **pooled** control set (`all_controls` = 40 probes + 609 codewords + 21 genomic controls): ~~NOT FLAT~~ **not flat pooled**, −0.0744 SD [−0.1306, −0.0182], p = 0.0145 naive; N5 removes it (+0.0038, p = 0.715), **N2 does not** (−0.0642, p = 0.0124) — so it rests on the **codewords (−0.0604) and genomic controls (−0.0307)**. Measured FPR **9–16 %** against a 5 % nominal (9–15 % on the four count-based responses). All from `results/phase3/a7_summary.csv` (09:06) | **FILL IN — and it is a negative result about the platform. Do NOT fill this in as "the negative-control probes are not flat": that states the opposite of the pre-registered test's own result. Forbidden-claims item 2.** |
