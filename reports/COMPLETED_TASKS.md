...
| 78 | **Freeze document corrected before tagging** | `reports/PREREG_PHASE8.md` | **5 corrections.** It carried my "roughly doubles" overstatement in 2 places and 3 **pre-C6** A7 values. Frozen values re-read by me from `a7_summary.csv` (09:06): codewords **-0.0604** (was -0.0549), genomic **-0.0307** (was -0.0337), probes **-0.0225** p=0.129 (was -0.0177). ~~Residual stale digits: **0**~~ ***[CORRECTED 2026-08-27 (remediation pass) — this claim was FALSE, and it was mine. "Residual stale digits: 0" was true only of the three patterns I actually searched (codeword / genomic / probe amplitudes). It was not true of the file. An independent numbers audit (`reports/AUDIT_NUMBERS_FINAL.md` R8) found three further live pre-C6 sites in `PREREG_PHASE8.md`: **P2** (naive -0.070 p 0.023, N2 -0.061 p 0.020, N5 +0.007 p 0.41, biological +0.314/+0.077, plus the stale instruction "A7 must be re-run after 8.7" -- it was, at 09:06), **P3** (the FPR list's 0.127, and an instruction to quote **9-13 %** when frozen `a7_summary.csv` gives a clean-null subset of **9.1-14.5 %**), and **P24** ("~80 % intact" against the frozen **86.3 %**). All three are now carried as dated corrections **C-8 / C-9 / C-10** in `PREREG_PHASE8.md` §0.0, with the original wording left in place. The lesson recorded against myself: a residual-count is only as good as its pattern list, and a zero must state the patterns it was searched over.]*** |
| 83 | **`WRITING_PACK.md` built — 1,389 lines** | `reports/WRITING_PACK.md` | 53 marked re-derivations, a 33-item forbidden-claims checklist, 22 tabulated document disagreements with the authoritative file named, and a per-subsection source map with reproduce commands |
