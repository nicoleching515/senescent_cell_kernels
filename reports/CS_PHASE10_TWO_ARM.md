...
| **T10** | **The five-seed DeepScence consensus (PI decision D-A) is a new estimator and it is compute-bound.** | **Declared as a deviation**, with its reason (the frozen single-seed configuration is not seed-reproducible on H1: *r* = 0.372, Jaccard = 0.211 against M1's 0.9955 / 0.761) and its consequence (**the two arms now use different DeepScence estimators** — M1 keeps its frozen single-seed score, reproducible at *r* = 0.9999991 — and that asymmetry is itself reportable). The seed set is the five already frozen at PREREG §3.8; no new seed value was introduced. **All 35 runs completed** (§3.6). Measured rate under the Phase-10 load: **12–65 min per full-section run at 2 concurrent** — the hard maximum, 3 OOM-kill each other at this 57.7 GB cgroup — i.e. **~9 h of wall clock**, of which ~2 h was lost to T11. **The seed count was never silently reduced**; the producer names the estimator `consensus_k<n>` and writes `n_seeds` into every row so a partial panel could not have been mistaken for a complete one |
...
**Figure guard, before and after.** `python3 code/check_figures_guard.py` reported
**`OK: all 52 committed figures match`** before Phase 10 began, and **`OK: all 52 committed
figures match`** again after Figures 5 and 6 were written — the guard verifies the files in
its manifest and new files are outside it, so it stays green while new work is in progress and
still catches any change to a committed figure. **The manifest was re-snapshotted exactly once,
at the end of the phase: 52 → 58 entries**, the six new ones being
`figure5.{png,pdf}`, `figure5_data.csv`, `figure6.{png,pdf}` and `figure6_data.csv`.
**No pre-existing figure was regenerated or modified by Phase 10.** After the snapshot the
guard reports **`OK: all 58 committed figures match`**.
