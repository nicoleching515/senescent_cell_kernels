...
values and the regenerated PNG is byte-identical to the committed one — **verify this at drafting time by
running `python3 code/check_figures_guard.py`; do not quote a hash from this document** [F].

***[Corrected 2026-08-27 (remediation pass) — **the hash is removed from this line rather than
replaced, and that is deliberate.**

**What was wrong.** The line quoted md5 `000f34051112aff4fed293fe7a5b25c2`, which was **not** the
hash of the file on disk. That string occurs in exactly two places in this repo:
`reports/CS_PHASE7_C1.md` L308, a **pre-C6** document recording a check made before the task-8.7
regeneration, and this line, which copied it from there. **It was read from a report, not from a
file — the exact failure mode the `[F]` mark exists to exclude** (`AUDIT_NUMBERS_FINAL.md` R3).

**Why it is not simply replaced with the on-disk value.** `figures/` is being regenerated by task
8.7 *while this correction is being written*, and the hash moved twice in one session:
`md5sum figures/figure4.png` gave **`d44fac63411d6c30a42c40894a287f17`** at 17:2x (matching
`results/phase3/m1_final_audit.txt` §5 and, by sha256, `figures/.committed_manifest.json`) and
**`fdead29871b61481e297951dfea75b3d`** minutes later, after `code/make_figure4.py` was re-run.
**Any literal hash typed into a drafting document during an active regeneration is stale on
arrival** — which is how the first one got here. The audit offered "drop the md5 **or** replace it
with the on-disk one"; under an active regeneration, dropping it is the only stable choice.

**What to do instead.** Establish byte-identity by running the guard
(`python3 code/check_figures_guard.py`), which compares every committed figure against
`figures/.committed_manifest.json` and names any that moved. Quote its output, dated, not a hash.]***
