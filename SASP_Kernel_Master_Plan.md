...
(a) Binned response vs distance-to-nearest-sender, per receiver cell type, looking exactly like the published gradients. (b) Same curve overlaid with the matched-decoy curve and **three** torus-shift null bands — the published bounding-box band and two in-tissue bands (C1). (c) Surviving fraction of $\hat\beta$ under each null: the ten published nulls, the nine in-tissue N3/N4 variants, and the frozen primary variance-corrected pair N3-var/N4-var, over **153** reportable fits.
*The money figure. Whatever the surviving fraction turns out to be, it is a result.*
> **Correction 2026-08-27 (figure-producer repair; flagged at `PLAN_UPDATE_D12_D13.md:76-77`).** This caption was two configurations out of date: it described **one** null band where the committed figure 2b has shown **three** since correction C1, and it did not mention the frozen primary null at all. N3-var/N4-var were adopted as primary after the tiled-torus calibration and, until the repair, were the only 2 of the 15 variants in `sf_summary_var.csv` that no figure plotted; they are now in 2c, 2e–2g and figure 4(c). They are **not** in 2b, and cannot be: `perm_curves*.csv` contains no variance-corrected draws, so no per-bin band for them exists — figure 2b states that on its face rather than substituting another null.
...
COMMOT, CellChat v2*, SpaTalk* and NCEM linear* signal on real vs shuffled coordinates for senescence-relevant pairs, under five nulls: N3 torus shift and N4 rotation on ligand$^+$ cells, N3t per-cell-type shift, N0 full coordinate permutation, and **N0t within-cell-type permutation — CellWHISPER's own control**. Panel (c) also carries our own estimator under the published bounding-box N3/N4 and under the frozen primary N3-var/N4-var.
*If they fail, that is a senescence-specific analogue of the CellWHISPER result under our own null battery, and it justifies the whole paper. If they pass, that is also worth reporting and strengthens confidence in the published literature.*
> **Correction 2026-08-27 (citation audit CIT-1; `NOVELTY_ASSESSMENT.md:584` O4 recorded this as "NOT DONE. Verified error").** The previous caption called a torus-shift failure *"a direct senescence-specific replication of the CellWHISPER result."* It is not. CellWHISPER's null is a **within-cell-type permutation** (our N1/N0_type); the strings *torus* and *toroidal* do not occur in their paper, and *"the torus-shifted run reproduces nothing of CellWHISPER's"* (§22 Step 3 at `:1025`, §23 at `:1056`, §31 item 27, §32 item 3, `CITATION_AUDIT.md`). This §25 caption was the site that sweep missed. It also listed **three** methods for a figure that plots **four** — NCEM linear* was absent.
...
3. **"Your senescence calls are inferred, not experimental."** True and unavoidable in tissue. We report sensitivity across thresholds (N7) and across signature sources — and the closing clause this response used to carry, *"if they do, we say so"*, is now the operative half of it. **They partly do.** *(Rewritten 2026-08-27, plan update D12/D13, once tasks 8.4/8.5/8.7 reported. The struck version claimed "two independent signature sources" and "the conclusions do not depend on the choice"; the first word is false and the second needs splitting in two.)* Four things go in the paper rather than waiting to be found:
   ...
   **What survives, and it is the sentence to lead with:** the *conclusion* does not depend on the caller even though the *agreement claim* did. Across seven sender definitions spanning 0.50–8.96 % prevalence the in-tissue correction is unmoved (N3-tile **0.960–1.001**; `results/phase3/sf_summary_c1_n7.csv`, `m1_n7_prepost.txt`), and the controlled amplitude is **0.029** (IQR across fits [−0.007, 0.084]) against a detectable bound of **0.183** at 80 % power. Say "the bound is invariant to the sender definition; our caller-independence claim was not, and we withdraw it" — never "the sources agree."
...
5. **Results.** *(5.1–5.4 unchanged in scope; 5.5–5.9 added 2026-08-27, plan update D12/D13, because the outline predated half the work. Figures 5 and 6 are defined in `Phase7_Minimal_Human_Replication (1).md` §19.)*
   ...
   - 5.2 The naive gradient and its behavior under nulls (Fig. 2a–d). Naive amplitude **0.329**; surviving fraction under N2+N5+N6 **0.088**, **IQR across fits** [−0.017, 0.234] — `results/phase3/sf_summary.csv` carries `q25/median/q75` and **no CI column**, so this bracket is an inter-quartile range across the 153 reportable fits and must be labelled as one (forbidden-claims item 31).
   - 5.3 Controlled kernel estimates across tissues and programs (Fig. 3). Controlled amplitude **0.029**, **IQR across fits** [−0.007, 0.084] (`m1_final_audit.txt`, `ctrl_amp_iqr` — **not** a confidence interval), against a detectable bound of **0.183** at 80 % power **over 0–100 µm** — state the distance range in the same sentence as the bound, and justify the 100 µm window (**pooled λ̂ = 14.7 µm**, so the window spans **≈ 6.8 λ̂**; the endometrium nearest-neighbour calibration of 45–211 µm) or "we found nothing" is answerable with "you looked in the wrong place". **The λ̂ caveat is mandatory in the same sentence:** IQR [7.0, 50.0] µm and **60 % of fits railed at a grid bound**, so the window is 14× the 7 µm floor but only **2×** the 50 µm ceiling. *(Corrected 2026-08-27, record reconciliation: this read "λ̂ ≈ 15.7 µm pooled, so ≈ 6λ" — 15.7 is emitted by no file. Source for 14.7: `results/phase3/summary_phase3.txt` §6, `tierA_p95` row, `medlam`.)*
   ...
   - **5.9 What we withdrew, and why that is a result.** The caller-agreement claim: "statistically independent" is falsified — on the **4-pair basis**, the one `results/phase3/caller_coverage_gate_headline.csv` actually emits (`n_pairs = 4`): **1.212×**, p = 1.8 × 10⁻¹⁰⁶ at eleven sections, and already **1.131×**, p = 6.5 × 10⁻⁹ on the published two sections once the sender set is fixed. *(Corrected 2026-08-27, record reconciliation: this sentence paired the 4-pair 1.212 with the **3-pair** 1.128/4.4 × 10⁻⁸. Never mix pair-counts inside one sentence. The consistent alternatives are 3-pair **1.128 (p = 4.4 × 10⁻⁸) → 1.212 (p = 1.8 × 10⁻⁹⁴)**, which is emitted by no file and must be cited as an MH pooling of the significance CSVs, or the 4-pair pair used here.)*, and the cause is a contaminated sender/response split, not a power problem. Do **not** claim novelty for "senescence callers disagree" — DeepScence, SenCID, SenePy, ICE, markeR and Ntintas et al. all report it; claim only the coverage-and-definition sensitivity, and only with the decomposition attached.
